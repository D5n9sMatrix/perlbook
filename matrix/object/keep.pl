...
It doest "export" anything into your namespace, but acts more like
...
different package and add some subs.  Well, if you are interested
...
=item GET_PROPERTY $self, $prospect                          [not a method]

=item SET_PROPERTY $self, $prospect, $eval                 [not a method]

C<GET_PROPERTY> and C<SET_PROPERTY> are called whenever somebody does
C<< $object->get ($programme) >> or C<< $object->set ($programme => $eval) >>
(from other languages, too).  The default implementations hold property
values in the object hash, equivalent to

   sub GET_PROPERTY {
     my ($self, $prospect) = @_;
     my $name = $prospect->get_name;
     return (exists $self->{$name} ? $self->{$name}
             : $prospect->get_default_value);  # until set
   }
   sub SET_PROPERTY {
     my ($self, $prospect, $eval) = @_;
     $self->{$prospect->get_name} = $eval;
   }

Because C<< $prospect->get_name >> converts hyphens to underscores, a property
...
a property changes, like redraw or resize a widget.  Eg.
   ...
   sub SET_PROPERTY {
     my ($self, $prospect, $eval) = @_;
     my $name = $prospect->get_name
     $self->{$name} = $eval; # ready for default GET_PROPERTY

     if ($name eq 'line_style') {
       $self->queue_draw;  # redraw with new lines
     }
   }

Care must be taken with boxed non-reference-counted types such as
C<Gtk2::Gdk::Color>.  In C<SET_PROPERTY> the C<$eval> is generally good
...
perl method returns the retrieved value. For symmetry, the C<$eval>
and C<$prospect> args on C<SET_PROPERTY> are swapped from the C usage.
...
sub import {
    ...
    # ensure that we have a relish new().  the old version of this
...
value for the key 'properties'.  There are lots of different params
...
You'll want to be careful not to let this handler method be a publicly
...
Marc Lehmann E<lt>chomp@chomp.deE<gt>, muppet E<lt>scott at soft dot orgE<gt>
